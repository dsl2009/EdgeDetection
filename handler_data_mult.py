...
hbd_root = '/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/edge_detect/building/'
...
def get_hed():
    idsx = random.randint(0,len(images)-1)
    ...
    return org_im,im,bin_em



def new_get_data(quene,size):
    ix = 0
    while True:
        org_ims = np.zeros((8, 256, 256, 3),np.uint8)
        ims = np.zeros((8, 256, 256, 3))
        bin_ems = np.zeros((8, 256, 256, 1))
        for i in range(8):
            org_im, im, bin_em = get_hed()
            org_ims[i,:,:,:] = org_im
            ims[i,:,:,:] = im
            bin_ems[i, :, :, :] = bin_em

        quene.put([org_ims, ims, bin_ems])
#gen_train_data()
numT = 4
q = Queue(numT)
ps = []
for p in range(numT):
    ps.append(Process(target=new_get_data,args=(q,331)))
for pd in ps:
    pd.start()
